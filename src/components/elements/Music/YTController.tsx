...
const YouTubeController = () => {
  ...
  const seekTo = (time: number, seekEnd = true) => {
    ...
    if (!seekEnd) {
      if (beforePlaying.current == null) {
        beforePlaying.current =
          iFrame.getInternalPlayer().getPlayerState() === 1;
      }
  ...
  onProgress.current = (time: number) => {
    ...
    const currentMeasureCount = Math.floor(
      delayedTime / beatTime / mainScore.beatCount,
    );
